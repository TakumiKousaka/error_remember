...
from flaskdb.models import User, Item, Order, Error
from flaskdb.forms import LoginForm, AddItemForm, SearchSolveForm, CheckOutForm, ErrorForm
...
def errorform():
    ...
    if form.validate_on_submit():
        ...
        flash("An code was added.", "info")
        return redirect(url_for("app.errorform"))
...
def searchitem():
    ...
    form = SearchSolveForm()

    if form.validate_on_submit():
        itemlist = Error.query.filter(Error.errordetail.like("%" + form.errordetail.data + "%")).all()
